...
const defaults: Required<TrackerOptions> = {
  lowScoreThreshold: 0.1, highScoreThreshold: 0.5, newTrackThreshold: 0.6,
  minHits: 2, matchIouThreshold: 0.3, lowMatchIouThreshold: 0.2,
  maxLostMs: 1000, largeGapMs: 2000, maxDetections: 100, maxTracks: 200,
};
type Entry = { id: number; classId: number; state: Track['state']; filter: GaussianState; firstMs: number; lastSeenMs: number; hits: number; score: number | null };
...
export function createTracker(input: TrackerOptions = {}): Tracker {
  ...
  return {
    update(inputFrame: TrackingFrame, updateOptions: UpdateOptions = {}): TrackingResult {
      ...
      // 所有计算在新对象上；成功构建结果后才提交时钟、ID 与轨迹。
      let working = entries.map(entry => ({ ...entry, score: null, filter: { mean: [...entry.filter.mean], covariance: entry.filter.covariance.map(row => [...row]) } } as Entry));
      ...
      const associate = (candidates: Entry[], observations: typeof high, threshold: number) => {
        const similarities = candidates.map(entry => observations.map(({ detection }) => entry.classId === detection.classId ? iou(toBox(entry.filter), detection.box) : -1));
        for (const [row, col] of assign(similarities, threshold)) { matches.set(candidates[row], observations[col].detection); used.add(observations[col].index); }
      };
      associate(working.filter(entry => entry.state !== 'tentative'), high, options.matchIouThreshold);
      associate(working.filter(entry => entry.state === 'tracked' && !matches.has(entry)), low, options.lowMatchIouThreshold);
      ...
      for (const { detection, index } of high) {
        ...
        working.push({ id: localNextId++, classId: detection.classId, state: options.minHits === 1 ? 'tracked' : 'tentative', filter: initialize(detection.box), firstMs: time, lastSeenMs: time, hits: 1, score: detection.score });
      }
      const tracks = working.map(entry => snapshot(entry, time));
      removed.sort((a, b) => a.id - b.id);
      const updateEnd = now();
      const result: TrackingResult = {
        generation, timestampMs: time, tracks, removed, droppedDetections,
