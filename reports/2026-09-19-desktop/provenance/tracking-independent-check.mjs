...
const sdkRoot='F:/git/00_chenmohan/github/web-sdk-PP-Tracking';
const {createTracker}=await import(pathToFileURL(sdkRoot+'/dist/index.js').href);
const inputPath='reports/tracking/2026-09-18-feasibility/inputs.json';
...
await fs.writeFile('.tmp/tracking-independent-check.json',JSON.stringify(report,null,2)+'\n');
console.log(JSON.stringify({scenes:reports.length,updates:reports.reduce((n,s)=>n+s.outputs.length,0),assertions:'通过',evidence:'.tmp/tracking-independent-check.json'}));
