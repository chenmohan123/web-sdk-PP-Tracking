...
- 本仓库属于单 SDK；生产运行时框架无关。默认入口为 CPU/main 跟踪算法，可选 `./reid` 子入口提供 WASM/WebGPU 特征提取，仅 load 会话阶段动态加载 ORT。
- 模型身份与来源以 `models/pplcnet-reid/0.1.0/{model,sources}.json` 为唯一注册数据，不在运行时代码重复维护；保持 ORT 1.27.0 为可选 peer，算法调用者无需安装。
