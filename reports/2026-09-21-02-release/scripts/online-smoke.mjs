...
const out = '.tmp/tracking-02-release/online';
...
try {
  await page.goto('https://chenmohan123.github.io/web-sdk-PP-Tracking/');
  assert(await page.evaluate(() => isSecureContext));
  assert.match(await page.locator('.brand-note').textContent(), /v0\.2\.0-rc\.0/);
  assert.equal(await page.getByRole('link', { name: 'npm RC', exact: true }).getAttribute('href'), 'https://www.npmjs.com/package/web-sdk-pp-tracking/v/0.2.0-rc.0');
  for (const algorithm of ['bytetrack', 'ocsort', 'deepsort']) {
    ...
    assert.match(await page.getByTestId('frame').textContent(), /1\//);
  }
  report.checks.push('线上 HTTPS、RC品牌/npm链接及三算法单步');
  report.url = page.url();
