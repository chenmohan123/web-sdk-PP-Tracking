...
Local required checks establish only locally-compliant status; the offline checker does not verify remote Rulesets, publication or hosting. Full verify and package consumption evidence are in the [2026-09-19 validation record](../../reports/2026-09-19-release-candidate/README.md); final package identity is in the [package report](../../reports/2026-09-19-release/README.md). Use the [release checklist](release-checklist.md) for remote evidence. Archive validation pins original file hashes, rejects changed samples and requires the current build to match the measured entry hash. It does not regenerate trusted hashes or repeat historical performance measurements. Each record is preserved separately without overwriting historical desktop or MOT17 reports.
