...
for (const [format, extension] of [['esm', 'js'], ['cjs', 'cjs']]) {
  ...
  await build({ entryPoints: ['src/motion/index.ts'], bundle: true, platform: 'neutral', format, target: 'es2022', outfile: `dist/motion/index.${extension}` });
}
// JSON 只打入运行时 bundle；声明先输出到临时目录，避免把模型目录带入包。
await rm('.tmp/package-types', { recursive: true, force: true });
const types = spawnSync(process.execPath, ['node_modules/typescript/bin/tsc', '--target', 'ES2022', '--module', 'ESNext', '--moduleResolution', 'Bundler', '--resolveJsonModule', '--strict', '--skipLibCheck', '--declaration', '--emitDeclarationOnly', '--newLine', 'lf', '--rootDir', '.', '--outDir', '.tmp/package-types', 'src/index.ts', 'src/reid/index.ts', 'src/motion/index.ts'], { stdio: 'inherit' });
...
console.log('算法、ReID 与运动估计独立 ESM、CommonJS 和 NodeNext 类型声明构建完成。');
