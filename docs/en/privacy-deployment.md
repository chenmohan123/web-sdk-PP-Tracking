...
Pages uses official configure-pages/upload-pages-artifact/deploy-pages actions. Only its deployment job receives pages/id-token write permissions, with the github-pages environment and serialized concurrency. Remote governance and delivery for 0.1.0 are archived. This stage distributes models to both hubs but does not publish the local 0.2.0-alpha.0 SDK, npm package or hosted Demo.
