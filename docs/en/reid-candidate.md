# Optional ReID module (preview capability)

[中文](../zh-CN/reid-candidate.md) · [Home](../../README.en.md)

This preview module extracts 512-dimensional appearance vectors from decoded person crops for the existing DeepSORT strategy. It does not detect boxes, open videos/cameras or assign track IDs. Root tracking strategies remain CPU/main; feature extraction separately selects WASM or WebGPU.

Preview builds and tarballs expose `web-sdk-pp-tracking/reid` with independent ESM/CJS/types. The standard1.3.0 hybrid manifest declares both algorithm and model. Install 0.2.0-rc.2 as described on the homepage, then optional `onnxruntime-web@1.27.0`. Root consumers need no engine dependency; weights are not in the tarball.
...
After installing the RC and optional ORT, import the same-package subpath. Published0.1.0 does not expose this feature:
...
The current [distribution and integration report](../../reports/2026-09-21-reid-distribution/README.en.md) records real dual sources, public subpath and Demo acceptance. `node tests/reid-distribution-browser.mjs` tests both sources/backends through the release dist and requires the previous local RGBA resources. Demo image-and-box mode extracts features and advances tracking from a local picture and caller-supplied detection array; it has no automatic detector. The [real same-input evaluation](../../reports/2026-09-21-mot-reid/README.en.md) used0.2.0-alpha.0 and covers seven sequences/5316 frames/67639 detections. DeepSORT+PPLCNet IDF1 is45.4637%, below ByteTrack48.2922%; other metrics and full measured costs are in the report, without claiming overall gains from selected metrics. The current prerelease is rc.2; this model evidence retains its original stage identity. Rc.2 adds the independent motion-estimation lab, keeps ByteTrack as default and human ReID experimental. No appearance-disabled ablation was run, so results cannot be attributed solely to the model. Phones, Safari, Firefox, Workers, NPU and complete video/camera pipelines are unverified.
