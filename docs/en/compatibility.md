...
Local required checks establish only locally-compliant status. Remote Rulesets, release and hosting checks remain skipped. Version 0.1.0 is a release candidate; npm, GitHub Release and hosted Demo availability require dated remote receipts, not manifest URLs. Archive validation pins original file hashes, rejects changed samples and requires the current build to match the measured entry hash. It does not regenerate trusted hashes or repeat historical performance measurements. Candidate packaging and the current full verify have separate release evidence without overwriting historical desktop or MOT17 reports.
