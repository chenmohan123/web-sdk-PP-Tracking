...
SDK无网络请求、遥测、模型下载或持久缓存，也不内置特征提取模型。Demo读取本地文件并在内存计算；不上传检测框或 embedding，刷新即清空。紧凑输入序列与结果报告均由用户主动导出，包含原始框、时间和可能可关联个体的外观向量；结果报告还包含跟踪结果与实际参数，请按自己的分享范围保存。轨迹ID不代表真实身份，特征空间标识也不证明向量来源。
