...
BoT-SORT adds original synthetic camera translation, an explicit motion-failure policy and optional appearance gates. It does not estimate image motion. Versioned input exports preserve the algorithm and applied options; re-import restores their meaning. Missing/invalid motion is rejected without changing the current session.
...
Omitting `algorithm` selects ByteTrack; `algorithm: 'ocsort'` selects OC-SORT; `algorithm: 'deepsort'` also requires a `featureSpace`, with matching IDs and vectors on every frame/detection. `algorithm: 'botsort'` requires frameId/motion and supports explicit motion-failure and optional appearance settings. ByteTrack and BoT-SORT use low-score continuation; strategy-specific options are mutually exclusive. All results report the actual `algorithm`. Seek requires reset followed by ordered replay.
...
`verify` builds the SDK, checks types and units, consumes all four algorithms from the actual npm tarball through ESM/CJS and declarations, builds the Demo and both examples, then tests real browser interactions. Screenshots and outputs go to `.tmp/browser/`.
