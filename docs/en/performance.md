...
DeepSORT `associationMs` includes appearance nearest-neighbour galleries, Mahalanobis gating, cascade matching and IoU fallback. Cost varies with track count, detection count, vector dimension and gallery capacity. It excludes caller-side ReID inference, image cropping, preprocessing and embedding generation, so it is not end-to-end appearance-pipeline time. Current evidence uses synthetic vectors and establishes no real-appearance accuracy or new cross-device performance claim.
